...
| show_days                    | number/range | **Optional** |          | E.g. `0` to only show items that are due today, overdue or have no due date. If an integer is specified overdue tasks AND any occuring in the next `X` days will be shown; e.g. `7` will display over due tasks and any in the next 7 days. If a range is specified show only tasks with a due date in that range; e.g. `1..10` would show tasks due in the next 10 days, but not overdue tasks, or tasks due today. If not specified, shows all items.                                                                                                                      |
